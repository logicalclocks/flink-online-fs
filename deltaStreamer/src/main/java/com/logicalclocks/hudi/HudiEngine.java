...
public class HudiEngine {
  ...
  protected static final String KAFKA_SOURCE = "com.logicalclocks.hudi.DeltaStreamerKafkaSource";
  protected static final String SCHEMA_PROVIDER = "com.logicalclocks.hudi.DeltaStreamerSchemaProvider";
  protected static final String DELTA_STREAMER_TRANSFORMER = "com.logicalclocks.hudi.DeltaStreamerTransformer";
  ...
  public void streamToHoodieTable(FeatureGroup featureGroup, Integer minSyncIntervalSeconds,
                                  Map<String, String> writeOptions) throws Exception {
    Map<String, String> hudiWriteOpts = setupHudiWriteOpts(featureGroup, HudiOperationType.UPSERT, null);
    ...
    hudiWriteOpts.put("targetBasePath", featureGroup.getLocation());
    hudiWriteOpts.put("targetTableName", utils.getFgName(featureGroup));
    hudiWriteOpts.put("featureGroupLocation", featureGroup.getLocation());
    hudiWriteOpts.put("onlineTopicName", featureGroup.getOnlineTopicName());
    hudiWriteOpts.put("avroSchema", featureGroup.getAvroSchema());
    hudiWriteOpts.put("sourceOrderingField", new ArrayList<>(
        JavaConverters.asJavaCollection(utils.getPrimaryColumns(featureGroup))).get(0));
    hudiWriteOpts.put("checkpointLocation", constructCheckpointPath(featureGroup, null, "delta_stream_"));

    deltaStreamerConfig.streamToHoodieTable(hudiWriteOpts, writeOptions,
        minSyncIntervalSeconds, SparkEngine.getInstance().getSparkSession());
  }
